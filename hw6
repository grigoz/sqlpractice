...
sql_str = "SELECT movieid , count(rating) as ratings_num, avg(rating) as ratings_avg INTO movies_top FROM public.ratings GROUP BY movieid"
...
engine = engine = create_engine('postgresql://postgres:@{}:{}'.format(os.environ['APP_POSTGRES_HOST'], os.environ['APP_POSTGRES_PORT']))
...
top_5_tags = tags_df.groupby(by=['tags'])['tags'].count().sort_values(ascending=False).head(5)
